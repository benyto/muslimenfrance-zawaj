...
export function ReportForm({ profileId, onDone }: { profileId: string; onDone: () => void }) {
  ...
  return (
    <form onSubmit={submit} className="mt-3 flex flex-col gap-3 rounded-xl border border-neutral-200 p-4 dark:border-neutral-800">
      <select
        value={reason}
      ...
      <textarea
        ...
        className="rounded-xl border border-neutral-200 bg-white px-3 py-2 text-sm dark:border-neutral-800 dark:bg-neutral-900"
      />
      {createReport.isError && <p className="text-sm text-red-600">{(createReport.error as Error).message}</p>}
      <div className="flex gap-2">
        ...
        <button type="button" onClick={onDone} className="rounded-xl px-4 py-2 text-sm text-neutral-500">
          Annuler
        </button>
      </div>
    </form>
