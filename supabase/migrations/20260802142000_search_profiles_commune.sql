-- Repoint get_profile_detail() / search_profiles() at communes_fr instead
-- of the now-dropped cities table, and add department/region filters to
-- search_profiles(). Postgres won't let `create or replace function`
-- change a TABLE-returning function's output columns, so both are dropped
...
begin
  ...
  select
    ...
    p.commune_insee_code, cf.nom_standard, dep.name, reg.name,
...
drop function public.search_profiles(text, uuid, text, int, int, int, int);
