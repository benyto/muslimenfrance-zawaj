-- Fix: search_profiles()/get_profile_detail() raised "structure of query
-- does not match function result type" the first time they actually
-- returned a row with a commune attached. Same class of bug as the earlier
-- varchar/text RETURN QUERY mismatch (migration 20260802100000) — the
-- pre-existing communes_fr table (not created by our migrations) declares
-- nom_standard as varchar, not text, and RETURN QUERY requires an exact
-- type match against the declared TABLE return type.
...
revoke execute on function public.search_profiles(text, text, text, text, text, int, int, int, int) from anon, authenticated, public;
grant execute on function public.search_profiles(text, text, text, text, text, int, int, int, int) to authenticated;
